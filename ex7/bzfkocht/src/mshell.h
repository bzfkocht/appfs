/**@file      
 * @brief     Memory allocation Shell
 * @author    Thorsten Koch
 * @date      04Jan2014
 * @copyright 2007-2014 by Thorsten Koch <koch@zib.de>
 *  
 ...
 *
 * Define NO_MSHELL to disable the mshell functionality.
 */
#ifndef _MSHELL_H_
#define _MSHELL_H_  
...
/*lint -sem(  mem_calloc, nulterm(3), 1n > 0 && 2n && 3p, @P == malloc(1n * 2n)) */
extern void*  mem_calloc(size_t, size_t, const char*, const int);
/*lint -sem(  mem_realloc, custodial(1), nulterm(3), 1p && 2n && 3p, @P == malloc(2n)) */
extern void*  mem_realloc(void*, size_t, const char*, const int);
/*lint -sem(  mem_strdup, nulterm(1), nulterm(2), 1p && 2p, @P == malloc(1P) && nulterm(@p)) */
extern char*  mem_strdup(const char*, const char*, const int);
/*lint -sem(  mem_free, custodial(1), nulterm(2), 1p && 2p) */
extern void   mem_free(void* p, const char*, const int);
